...
def index():
    ...
    rows = db.execute(
        "SELECT name, symbol, SUM(amount) AS shares, share_price FROM purchases WHERE id = :user_id GROUP BY symbol HAVING shares > 0", user_id=session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not stock_info or stock_info == None:
            return apology("invalid or empty symbol", 400)
        if not shares:
            return apology("Must be a number greater than zero", 400)
        if shares.isdecimal():
            shares = float(shares)
        else:
            return apology("Must be a number greater than zero", 400)
        ...
        share = db.execute("SELECT SUM(amount) AS amount FROM purchases WHERE id = :id AND symbol = :symbol GROUP BY symbol",
        ...
        if ((rquotev * number) > cash):
            ...
            db.execute("INSERT INTO purchases (id, name, symbol, amount, share_price, time) VALUES(?, ?, ?, ?, ?, ?)",
                       session["user_id"], name, stock, number, rquote["price"], time)
...
def history():
    ...
    # Select all information from current user
    rows = db.execute("SELECT * FROM purchases WHERE id=:id", id=session["user_id"])

    # Set empty array
    history = []

    # Iterate through all information from current user and store them in an array
    for row in rows:
        history.append(rows)
...
def register():
    ...
    if request.method == "GET":
        ...
        if not request.form.get("username"):
            return apology("Type an username", 400)
        elif not request.form.get("password"):
        ...
        if len(add) == 0:
            db.execute("INSERT INTO users (username, hash) VALUES (:user, :password)", user=request.form.get(
            ...
            return apology("Username already in use. Try another one", 400)
...
def sell():
    ...
    if request.method == "POST":
        ...
        stock_info = request.form.get("symbol")
        # Turn the information in CAPS if needed
        ...
        if info[0]["amount"] <= 0 or info[0]["amount"] < number:
            return apology("You don't have this number of shares to sell", 400)
        ...
        stocks = db.execute(
            "SELECT symbol, SUM(amount) AS amount FROM purchases WHERE id = :id GROUP BY symbol HAVING amount > 0", id=session["user_id"])
...
def change():
    ...
    if request.method == "POST":
        ...
        # If password is correct:
            ...
            if new == newp:
                hash = db.execute("UPDATE users SET hash = :hash WHERE id = :id",
